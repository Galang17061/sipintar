var histori_ujian_data = {
 ...
 exec_save: function () {
  var data = histori_ujian_data.get_post_data();
  ...
  $.ajax({
   ...
   processData: false,
   async: false,
   url: url.base_url(histori_ujian_data.module()) + 'save',
   error: function () {
   ...
   success: function (resp) {
    ...
    histori_ujian_data.resetformInput();
   }
  });
 },
...
//   message.showDialog('Proses Mendapatkan Data..');
   ...
   $.ajax({
    ...
    dataType: 'html',
    async: false,
    url: histori_ujian_data.module() + '/search',
    success: function (resp) {
...
//     message.closeDialog();
 ...
 remove: function (id) {
  ...
  html += '<button class="btn btn-primary" onclick="histori_ujian_data.exec_remove(' + id + ')">Ya</button> &nbsp;';
 ...
 exec_remove: function (id) {
  $.ajax({
   ...
   dataType: 'json',
   async: false,
   url: url.base_url(histori_ujian_data.module()) + 'remove' + '/' + id,
   error: function () {
   ...
   success: function (resp) {
    if (resp.is_valid) {
     ...
     setTimeout(histori_ujian_data.reloadPage(), 1000);
    } else {
 ...
 detailUjian: function(ujian){
  window.open(url.base_url(histori_ujian_data.module()) + 'detailUjian' + '/' + ujian);
 }
};
